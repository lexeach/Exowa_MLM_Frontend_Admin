...
const UserBankKyc = () => {
    ...
    let { token, userData } = useSelector((state) => state);
    const [bankKyc, setbankKycData] = useState([])
    const bankKycData = bankKyc?.filter((item) => item.userid === userData.userid);
        ...
        <>

            {
                loading ?
                    <Loader />
                    :
                    <div className='w-full border-[1px] border-customBorderColor rounded-xl shadow-sm h-[calc(100vh-140px)] overflow-auto'>
