...
const UserLevels = () => {
    ...
    let { userData, token } = useSelector((state) => state);
    const [userLevel, setuserLevelData] = useState([])
    const userLevelData = userLevel?.filter((item) => item.userid === userData.userid);
    ...
    const fatchData = async () => {
        ...
        try {
            ...
            const response = await ApiCall(headers, `/admin/user_levels`);
            if (response.status === 200) {
                setuserLevelData(response.data.data)
                setloading(false)
            }
        } catch (error) {
    ...
    }, [loading, userLevelData]);

    const formattedUserLevelData = userLevelData?.map((item, index) => ({
        "S.No": index + 1,
        "User Id": item.userid || 'N/A',
        "User Name": item.user_name || 'N/A',
        "Income(₹)": formatAmount(item.transaction_sum) || 'N/A',
        "Power(₹)": formatAmount(item.power) || 'N/A',
        ...
        <>

            {
                loading ?
                    <Loader />
                    :
                    <div className='w-full border-[1px] border-customBorderColor rounded-xl shadow-sm h-[calc(100vh-140px)]'>
                            ...
                            {(userLevelData?.length > 0) ? (
                                <>
                                    <div className='absolute right-[3px] md:right-[337px] top-6 cursor-pointer z-20'>
                                        <CustomDownloadFile
                                            data={formattedUserLevelData}
                                            filename="User-Levels-Data"
                                        />
                                    </div>
                                    <table ref={tableRef} className='display' style={{ width: "100%" }} >
                                        ...
                                        <tbody>
                                            {formattedUserLevelData?.map((item, index) => {
                                                return (
                                                    <tr key={index}>
                                                        <td>{index + 1}</td>
                                                        <td>{item['User Id']}</td>
